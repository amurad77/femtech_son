...
def products(request):
    products = ProductsCategory.objects.all().order_by('-id')

    page_num = request.GET.get('page', 1)

    paginator = Paginator(products, 12) # 6 employees per page
...
def single_product(request, slug):
    products = get_object_or_404(ProductsCategory, slug = slug)
    products_list = Products.objects.filter(products=products)

    page_num = request.GET.get('page', 1)

    paginator = Paginator(products_list, 12) # 6 employees per page
